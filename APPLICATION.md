...
核心交付包括 Unicode 17.0.0 DUCET 数据生成、规范等价处理、contraction 与
expansion、隐式权重、Primary 至 Identical 多级比较、Non-Ignorable/Shifted
策略、二进制 Sort Key、可解释比较轨迹、CLI，以及复用同一 MoonBit 核心的
浏览器 Collation Lab。项目将以 Unicode 官方 CollationTest 为验收基准，并在
native、js、wasm、wasm-gc 后端执行检查与测试。
...
MoonBit UCA、DUCET、Collator 或通用 Sort Key 实现。仓库将持续维护独立的
生态查重报告，并明确声明相邻项目和功能边界。

项目采用 Apache-2.0 许可证；Unicode 数据保留 Unicode License v3 与来源说明。
所有功能通过 Issues、独立分支、Pull Requests、Actions、Milestone 和 Release
形成可追踪的开发与验收链。
